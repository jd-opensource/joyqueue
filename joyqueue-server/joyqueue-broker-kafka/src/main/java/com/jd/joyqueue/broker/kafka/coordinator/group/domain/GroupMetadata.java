 ...
 */
package com.jd.joyqueue.broker.kafka.coordinator.group.domain;
 ...
 * date: 2018/12/4
 */
public class GroupMetadata extends com.jd.joyqueue.broker.coordinator.group.domain.GroupMetadata {
    ...
    public com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata getMember(String memberId) {
        if (StringUtils.isBlank(memberId)) {
            return null;
        }
        return (com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata) getMembers().get(memberId);
    }
    ...
    public void addMember(com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata member) {
    ...
    public List<com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata> getNotYetRejoinedMembers() {
        List<com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata> result = Lists.newLinkedList();
        for (Map.Entry<String, com.jd.joyqueue.broker.coordinator.group.domain.GroupMemberMetadata> entry : getMembers().entrySet()) {
    ...
    public List<com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata> getAllMembers() {
        ConcurrentMap<String, com.jd.joyqueue.broker.coordinator.group.domain.GroupMemberMetadata> members = getMembers();
        if (MapUtils.isEmpty(members)) {
            return Collections.emptyList();
        }
        List<com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata> result = Lists.newArrayListWithExpectedSize(members.size());
        for (Map.Entry<String, com.jd.joyqueue.broker.coordinator.group.domain.GroupMemberMetadata> entry : members.entrySet()) {
    ...
    public String selectProtocol() {
        ...
        List<com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata> allMemberMetadata = getAllMembers();
        for (com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata memberMetadata : allMemberMetadata) {
    ...
    public boolean initNextGeneration() {
        List<com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata> memberMetadataSet = getNotYetRejoinedMembers();
    ...
    public Map<String, byte[]> currentMemberMetadata() {
        ...
        Iterator<Map.Entry<String, com.jd.joyqueue.broker.coordinator.group.domain.GroupMemberMetadata>> iterator = getMembers().entrySet().iterator();
        while (iterator.hasNext()) {
            Map.Entry<String, com.jd.joyqueue.broker.coordinator.group.domain.GroupMemberMetadata> entry = iterator.next();
            String memberId = entry.getKey();
            com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata memberMetadata = (com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMemberMetadata) entry.getValue();
    ...
    private void assertValidTransition(GroupState targetState) {
        if (!com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMetadata.ValidPreviousStates.get(targetState).contains(state)) {
            throw new IllegalStateException(String.format("Group %s should be in the %s states before moving to" +
                    ...
                    getId(), com.jd.joyqueue.broker.kafka.coordinator.group.domain.GroupMetadata.ValidPreviousStates.get(targetState), targetState, state));
        }
    }
