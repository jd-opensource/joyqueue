...
import com.jd.joyqueue.broker.jmq2.command.ClusterRole;
...
import com.jd.joyqueue.broker.jmq2.command.Permission;
import com.jd.joyqueue.broker.jmq2.command.RetryType;
import com.jd.joyqueue.broker.jmq2.command.SyncMode;
import com.jd.joyqueue.broker.jmq2.config.JMQ2Config;
import com.jd.joyqueue.broker.jmq2.converter.TopicConverter;
import com.jd.joyqueue.broker.jmq2.entity.GetClusterEntity;
 ...
 * date: 2018/8/27
 */
public class ClusterHandler implements JMQ2CommandHandler, Type, BrokerContextAware {
    ...
    public void setBrokerContext(BrokerContext brokerContext) {
        ...
        this.config = new JMQ2Config(brokerContext.getPropertySupplier());
    ...
    protected GetClusterEntity doGetCluster(String app,DataCenter clientDataCenter) {
        String[] appGroup = app.split(JMQ2Consts.APP_GROUP_SPLIT);
        ...
        if(producerPolicy.getNearby()&&!hasPredefinedWeight(producerPolicy)&& CollectionUtils.isNotEmpty(topicCluster.getGroups())){
          ...
          for(BrokerGroup bg:topicCluster.getGroups()){
              ...
              if(groupPerm.contain(Permission.WRITE)){
                 JMQ2Broker jmq2Broker= getBrokerGroupLeader(bg);
     ...
     **/
    public JMQ2Broker getBrokerGroupLeader(BrokerGroup brokerGroup){
        for(JMQ2Broker b:brokerGroup.getBrokers()){
    ...
    protected BrokerGroup convertToBrokerGroup(PartitionGroup partitionGroup, Producer.ProducerPolicy producerPolicy) {
        ...
        brokerGroup.setBrokerType(JMQ2Broker.BrokerType.JMQ);
        ...
        if (MapUtils.isNotEmpty(brokers)) {
            for (Map.Entry<Integer, Broker> entry : brokers.entrySet()) {
                ...
                JMQ2Broker jmq2Broker = new JMQ2Broker();
                jmq2Broker.setId(broker.getId());
                jmq2Broker.setIp(broker.getIp());
                jmq2Broker.setPort(broker.getPort());
                jmq2Broker.setType(JMQ2Broker.BrokerType.JMQ);
                jmq2Broker.setLocation(JMQ2Broker.Location.IDC);
                jmq2Broker.setRole(partitionGroup.getLeader().equals(broker.getId()) ? ClusterRole.MASTER : ClusterRole.SLAVE);
                jmq2Broker.setRetryType(broker.getRetryType().equals(Broker.DEFAULT_RETRY_TYPE) ? RetryType.REMOTE : RetryType.DB);
                jmq2Broker.setSyncMode(SyncMode.ASYNCHRONOUS);
                jmq2Broker.setGroup(brokerGroup.getGroup() + "_" + brokerGroup.getGroup() + "_" + partitionGroup.getTerm());

                if (Broker.PermissionEnum.FULL.equals(broker.getPermission())) {
                    jmq2Broker.setPermission(Permission.FULL);
                } else if (Broker.PermissionEnum.WRITE.equals(broker.getPermission())) {
                    jmq2Broker.setPermission(Permission.WRITE);
                } else if (Broker.PermissionEnum.READ.equals(broker.getPermission())) {
                    jmq2Broker.setPermission(Permission.READ);
                } else if (Broker.PermissionEnum.NONE.equals(broker.getPermission())) {
                    jmq2Broker.setPermission(Permission.NONE);
                } else {
                    jmq2Broker.setPermission(Permission.NONE);
                }
                brokerGroup.addBroker(jmq2Broker);
            }
        }
