 ...
 */
@FetchHandler
public class GetMessageHandler implements JMQ2CommandHandler, Type, BrokerContextAware {
    ...
    protected PullResult doGetBroadcastMessage(Transport transport, Command command, Consumer consumer, Traffic traffic, GetMessage getMessage, org.joyqueue.domain.Consumer topicConsumer) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(pullResult.getBuffers())) {
            ...
            brokerMessages = messageConvertSupport.convert(brokerMessages, SourceType.JMQ.getValue());

            for (BrokerMessage brokerMessage : brokerMessages) {
                brokerMessage.setPartition((short) (brokerMessage.getPartition() + 1));
                brokerMessage.setMsgIndexNo(brokerMessage.getMsgIndexNo() * JMQ2Consts.INDEX_LENGTH);
