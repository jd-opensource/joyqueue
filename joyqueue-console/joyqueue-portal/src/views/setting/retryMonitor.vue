<template>
  <div>
    ...
    <!--批量清理-->
    <my-dialog :dialog="cleanAllRetryMessage" @on-dialog-confirm="cleanAllConfirm()" @on-dialog-cancel="editCancel()">
      <grid-row class="mb10">
        ...
        <grid-col offset="4" style="margin-top: 10px">
          <d-tooltip  content="删除过期时间以前的重试消息"><Icon name="help-circle" class="help-icon" size="16" color="#4F8EEB" />
          </d-tooltip>
        </grid-col>
      </grid-row>
    </my-dialog>
    <my-dialog :dialog="cleanSingleConsumerDialog" @on-dialog-confirm="cleanSingleConsumerConfirm()" @on-dialog-cancel="editCancel()">
      <grid-row class="mb10">
        ...
        <grid-col :span="16" class="val">
          <d-input v-model="cleanup.app"></d-input>
        </grid-col>
        <grid-col :span="6" class="label">过期时间:</grid-col>
        <grid-col :span="16" class="val">
          <d-date-picker
