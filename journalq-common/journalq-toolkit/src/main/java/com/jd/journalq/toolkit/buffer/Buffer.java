...
import com.jd.journalq.toolkit.buffer.bytes.BytesInput;
import com.jd.journalq.toolkit.buffer.bytes.BytesOutput;
import com.jd.journalq.toolkit.buffer.bytes.SwappedBytes;
 ...
 * implementation. The {@code Buffer} type is agnostic about the specific underlying storage implementation, but
 * different
 ...
 * It intentionally exposes methods that can be easily understood by any developer with experience with {@code
 * ByteBuffer}.
 * <p>
 ...
 * </pre>
 * <p>
 * Buffers implement {@link Reference} in order to keep track of the number of currently
 ...
 * by using a try-with-resources statement demonstrated above. Additional references to the buffer should be acquired
 * via
 * {@link Reference#acquire()} and released via
 * {@link Reference#release}. Once all references to a buffer have been released - including
...
public interface Buffer
        extends BytesInput, BufferInput, BytesOutput<Buffer>, BufferOutput<Buffer>, Reference, Closeable {

    /**
     * Returns the byte order.
     * <p>
     * For consistency with {@link java.nio.ByteBuffer}, all buffer implementations are initially in
     ...
     * <p>
     * By default, all buffers are read and written in {@link java.nio.ByteOrder#BIG_ENDIAN} order. This provides
     * complete
     * consistency with {@link java.nio.ByteBuffer}. To flip buffers to {@link java.nio.ByteOrder#LITTLE_ENDIAN} order,
     * this
     ...
     * implementation. As bytes are written to the buffer, the buffer's capacity may grow up to
     * {@link Buffer#maxCapacity()}.
     *
     ...
     * <p>
     * The position is an internal cursor that tracks where to write/read bytes in the underlying storage
     * implementation.
     ...
     * be within the bounds of the buffer, i.e. greater than {@code 0} and less than or equal to
     ...
     * Flips the buffer.
     * <p>
     * The limit is set to the current position and then the position is set to zero. If the mark is defined then it is
     * discarded.
     * <pre>
     *   {@code
     ...
     * the buffer's {@code position} being reset to the {@code position} of this buffer at the time the slice was
     * created.
     * <p>
     ...
     * @throws java.nio.BufferUnderflowException If the length of the remaining bytes in the buffer is less than {@code
     *                                           length}
     * @see Buffer#slice()
     * @see Buffer#slice(long)
     */
     ...
     *                                           bounds are determined by the buffer's {@link Buffer#limit()} rather
     *                                           than capacity.
     * @see Buffer#read(Bytes)
     * @see Buffer#read(long, Bytes, long, long)
     */
    @Override
    long read(Bytes bytes, long dstOffset, long length);
     ...
     */
    @Override
    long read(long srcOffset, Bytes bytes, long dstOffset, long length);
     ...
     */
    @Override
    long read(long srcOffset, byte[] bytes, long dstOffset, long length);
     ...
     * @return The read byte.
     * @throws IndexOutOfBoundsException If the given offset is out of the bounds of the buffer. Note that
     *                                   bounds are determined by the buffer's {@link Buffer#limit()} rather than
     *                                   capacity.
     * @see Buffer#readByte()
     */
    @Override
     ...
     * <p>
     * When the character is read from the buffer, the buffer's {@code position} will be advanced by
     * {@link Bytes#CHARACTER}.
     ...
     * <p>
     * When the integer is read from the buffer, the buffer's {@code position} will be advanced by
     * {@link Bytes#INTEGER}.
     ...
     * length of the string. If there are no bytes remaining in the buffer then a
     ...
     * @throws java.nio.BufferOverflowException If the given buffer's {@link Buffer#remaining()} bytes exceeds this
     *                                          buffer's
     *                                          remaining bytes.
     */
    @Override
     ...
     * Writes an array of bytes to the buffer.
     * <p>
     * The bytes will be written starting at the current position up to the given length. If the length of the byte
     * array
     * is larger than the provided {@code length} then only {@code length} bytes will be read from the array. If the
     * provided {@code length} is greater than the remaining bytes in this buffer then a
     ...
     * Writes an array of bytes to the buffer.
     * <p>
     * The bytes will be written starting at the given offset up to the given length. If the remaining bytes in the
     * byte array
     * is larger than the provided {@code length} then only {@code length} bytes will be read from the array. If the
     ...
     * <p>
     * When the short is written to the buffer, the buffer's {@code position} will be advanced by {@link Bytes#SHORT}
     * . If
     ...
     * <p>
     * The short will be written at the given offset. If there are less than {@link Bytes#SHORT} bytes remaining in the
     * buffer
     ...
     * <p>
     * When the integer is written to the buffer, the buffer's {@code position} will be advanced by
     * {@link Bytes#INTEGER}.
     * If less than {@link Bytes#INTEGER} bytes are remaining in the buffer then a
     * {@link java.nio.BufferOverflowException}
     * will be thrown.
     *
     ...
     * If less than {@link Bytes#FLOAT} bytes are remaining in the buffer then a
     * {@link java.nio.BufferOverflowException}
     * will be thrown.
     *
     ...
     * If less than {@link Bytes#DOUBLE} bytes are remaining in the buffer then a
     * {@link java.nio.BufferOverflowException}
     * will be thrown.
     *
     ...
     * <p>
     * The string will be written with a two-byte unsigned byte length followed by the UTF-8 bytes. If there are not
     * enough
