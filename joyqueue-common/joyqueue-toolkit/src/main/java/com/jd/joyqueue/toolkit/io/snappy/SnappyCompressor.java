...
final class SnappyCompressor {
    ...
    private static int[] findCandidate(byte[] input, int ipIndex, int ipLimit, int inputOffset, int shift,
        ...
        for (
                ipIndex += 1; ipIndex + bytesBetweenHashLookups(skip) <= ipLimit;
                ipIndex += bytesBetweenHashLookups(skip++)) {
            // hash the 4 bytes starting at the input pointer
            int currentInt = Systems.getInt(input, ipIndex);
            ...
            if (currentInt == Systems.getInt(input, candidateIndex)) {
                break;
            }
        }
    ...
    private static int[] emitCopies(byte[] input, final int inputOffset, final int inputSize, int ipIndex,
        ...
        do {
            ...
            int prevInt;
            if (Systems.FAST_ACCESS_SUPPORTED) {
                long foo = Systems.getLong(input, ipIndex - 1);
                prevInt = (int) foo;
                inputBytes = (int) (foo >>> 8);
            } else {
                prevInt = Systems.getInt(input, ipIndex - 1);
                inputBytes = Systems.getInt(input, ipIndex);
            }
        ...
        } while (inputBytes == Systems.getInt(input, candidateIndex));
        return new int[]{ipIndex, outputIndex};
    }
    ...
    private static int findMatchLength(byte[] s1, int s1Index, byte[] s2, final int s2Index, int s2Limit) {
        ...
        if (Systems.FAST_ACCESS_SUPPORTED) {
            ...
            while (s2Index + matched <= s2Limit - 4 && Systems.getInt(s2, s2Index + matched) == Systems
                    .getInt(s1, s1Index + matched)) {
                matched += 4;
            }

            if (NATIVE_LITTLE_ENDIAN && s2Index + matched <= s2Limit - 4) {
                int x = Systems.getInt(s2, s2Index + matched) ^ Systems.getInt(s1, s1Index + matched);
