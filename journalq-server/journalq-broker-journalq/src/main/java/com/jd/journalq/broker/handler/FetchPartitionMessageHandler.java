 ...
 * date: 2018/12/13
 */
public class FetchPartitionMessageHandler implements JournalqCommandHandler, Type, BrokerContextAware {
    ...
    public Command handle(Transport transport, Command command) {
        ...
        if (connection == null || !connection.isAuthorized(fetchPartitionMessage.getApp())) {
            logger.warn("connection is not exists, transport: {}, app: {}", transport, fetchPartitionMessage.getApp());
        ...
        for (Map.Entry<String, Map<Short, FetchPartitionMessageData>> entry : fetchPartitionMessage.getPartitions().rowMap().entrySet()) {
            ...
            for (Map.Entry<Short, FetchPartitionMessageData> partitionEntry : entry.getValue().entrySet()) {
                ...
                if (!checkResult.isSuccess()) {
                    logger.warn("checkReadable failed, transport: {}, topic: {}, app: {}, code: {}", transport, consumer.getTopic(), consumer.getApp(), checkResult.getJournalqCode());
