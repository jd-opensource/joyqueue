 ...
 * date: 2018/12/12
 */
public class CommitAckHandler implements JournalqCommandHandler, Type, BrokerContextAware {
    ...
    public Command handle(Transport transport, Command command) {
        ...
        if (connection == null || !connection.isAuthorized(commitAck.getApp())) {
            logger.warn("connection is not exists, transport: {}, app: {}", transport, commitAck.getApp());
        ...
        for (Map.Entry<String, Map<Short, List<CommitAckData>>> entry : commitAck.getData().rowMap().entrySet()) {
            ...
            for (Map.Entry<Short, List<CommitAckData>> partitionEntry : entry.getValue().entrySet()) {
                JournalqCode ackCode = commitAck(connection, topic, commitAck.getApp(), partitionEntry.getKey(), partitionEntry.getValue());
