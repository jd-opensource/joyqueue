 ...
 * date: 2018/12/19
 */
public class FetchProduceFeedbackHandler implements JournalqCommandHandler, Type, BrokerContextAware {
    ...
    public Command handle(Transport transport, Command command) {
        ...
        if (!checkResult.isSuccess()) {
            logger.warn("checkWritable failed, transport: {}, topic: {}, app: {}, code: {}", transport, fetchProduceFeedback.getTopic(), fetchProduceFeedback.getApp(), checkResult.getJournalqCode());
            return new Command(new FetchProduceFeedbackAck(CheckResultConverter.convertCommonCode(checkResult.getJournalqCode())));
        }
    ...
    protected FetchProduceFeedbackAck FetchProduceFeedback(Connection connection, FetchProduceFeedback fetchProduceFeedback) {
        ...
        try {
            ...
            fetchProduceFeedbackAck.setCode(JournalqCode.SUCCESS);
            ...
            return new FetchProduceFeedbackAck(JournalqCode.valueOf(e.getCode()));
        } catch (Exception e) {
            logger.error("fetch feedback exception, transport: {}, topic: {}, app: {}", connection.getTransport().remoteAddress(), fetchProduceFeedback.getTopic(), fetchProduceFeedback.getApp(), e);
            return new FetchProduceFeedbackAck(JournalqCode.CN_UNKNOWN_ERROR);
        }
    }
