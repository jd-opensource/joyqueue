 ...
 * date: 2018/12/19
 */
public class ProduceMessagePrepareHandler implements JournalqCommandHandler, Type, BrokerContextAware {
    ...
    public Command handle(Transport transport, Command command) {
        ...
        if (!checkResult.isSuccess()) {
            logger.warn("checkWritable failed, transport: {}, topic: {}, app: {}, code: {}", transport, produceMessagePrepare, produceMessagePrepare.getApp(), checkResult.getJournalqCode());
            return new Command(new ProduceMessagePrepareAck(CheckResultConverter.convertCommonCode(checkResult.getJournalqCode())));
        }
    ...
    protected ProduceMessagePrepareAck produceMessagePrepare(Producer producer, Connection connection, ProduceMessagePrepare produceMessagePrepare) {
        ...
        try {
            ...
            return new ProduceMessagePrepareAck(transactionId.getTxId(), JournalqCode.SUCCESS);
            ...
            return new ProduceMessagePrepareAck(JournalqCode.valueOf(e.getCode()));
        } catch (Exception e) {
            logger.error("produceMessage prepare exception, transport: {}, topic: {}, app: {}",
                    connection.getTransport().remoteAddress(), produceMessagePrepare.getTopic(), produceMessagePrepare.getApp(), e);
            return new ProduceMessagePrepareAck(JournalqCode.CN_UNKNOWN_ERROR);
        }
    }
